...
function sortLinked(list: Node | null) {
  let numMerges;
  let inSize = 1;
...
interface Node {
  ...
  y: number;
  prev: Node | null; // previous and next vertex nodes in a polygon ring
  next: Node | null;
...
export default function earcut(
  ...
  const hasHoles: boolean = Boolean(holeIndices && holeIndices.length > 0);
...
// The rest of the functions would follow the same pattern of adding type annotations.
// For brevity, I'll show a few more key functions and you can apply the same pattern to the rest:
